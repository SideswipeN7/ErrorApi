...
using Sample.LanguageExt.Api.Orders;
...
app.MapScalarApiReference("/scalar", options => options.WithTitle("language-ext sample"));
...
orders.MapPost("/{id:guid}/pay", (Guid id, decimal amount, IOrderService service) =>
        service.Pay(id, amount).ToNoContentResult())
    .WithName("PayOrder")
