...
orders.MapGet("/{id:guid}", async (Guid id, ISender sender) =>
    (await sender.Send(new GetOrder(id))).ToHttpResult());   // documents 404 - no attribute anywhere
```
...
4. Endpoints dispatch through `ISender` and map the awaited result with `.ToHttpResult()`.
